...
static const std::vector<ImageResInfo>image_info_list = {  //列表初始化+R标记 不需要对反斜杠进行转义处理
	...
	{"hit_vfx_left",_T(R"(Fantasy_Warrior\Hit_vfx.png)")},
	...
	{"enemy_attack_up_right",_T(R"(Martial_Hero\Attack_up.png)")},
	{"enemy_attack_down_right",_T(R"(Martial_Hero\Attack_down.png)")},

	{"enemy_attack_shift_right",_T(R"(Martial_Hero\Attack_shift.png)")},

	{"enemy_attack_up_2",_T(R"(Martial_Hero\Attack_up_2.png)")},
	...
	{"enemy_slash_vfx",_T(R"(Martial_Hero\Slash.png)")},

	{"smoke_vfx_left",_T(R"(Martial_Hero\Smoke.png)")},

	{"enemy_attack_up_vfx",_T(R"(Martial_Hero\vfx_attack_up.png)")},
	{"enemy_attack_down_vfx",_T(R"(Martial_Hero\vfx_attack_down.png)")},
	...
	for (int j = 0; j < y_num; j++) {
		for (int i = 0; i < x_num; i++) {
			...
			for (int y = y_top; y < y_bottom; y++) {
				for (int x = x_left; x < x_right; x++) {
					int idx_src = y * w + x;
					int idx_dst = y * w + x_right - (x - x_left)-1;
					dst_buffer[idx_dst] = src_buffer[idx_src];
				}
			}
		}
...
void ResourcesManager::load() {
	...
	flip_image("enemy_attack_up_right", "enemy_attack_up_left", 4);
	flip_image("enemy_attack_down_right", "enemy_attack_down_left", 4);

	flip_image("enemy_attack_shift_right", "enemy_attack_shift_left", 5, 2);

	flip_image("smoke_vfx_left", "smoke_vfx_right",9);
	flip_image("hit_vfx_left", "hit_vfx_right", 7);
