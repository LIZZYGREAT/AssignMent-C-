...
Player::Player() {
	...
	hurt_box->set_on_collide([&](int x) {
		decrease_hp(x);
		if (!is_attacking && is_on_floor() && is_taking_hit_cd_cmp && x > 0) {
			switch_state("take_hit");
		}
		});
